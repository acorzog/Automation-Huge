...
browser.addCommand("submitDataViaContactUsForm", function(firtsName, lastName, emailAddress, comments) {
	if(firtsName) {
		browser.setValue("[name='first_name']", firtsName);
	}
	if(lastName) {
...
describe('Test Contact Us form WebdriverUni', function() {
	...
	var res = request('GET', 'http://jsonplaceholder.typicode.com/posts/1/comments'); //Json
...
contactusDetails.forEach(function (contactDetail) {
  it.only('Should be able to submit a successful submission via contact us form', function(done) {
  	browser.submitDataViaContactUsForm('joe', 'Blogs', contactDetail.email, contactDetail.body); //to use JSON put the current name of the file
